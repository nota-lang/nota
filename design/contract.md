...
| R20 | Doc-state sugar + fully-ambient prelude | new (**pinned 2026-07-05**) — the R18e constructs demanded `%import { Toc, … } from "@nota-lang/prelude"` in every document (only `Tex`/`CodeInline`/`CodeBlock`/`Heading` were ambient), and labels/refs/footnotes had no bespoke syntax — `@Label`/`@Ref`/`@Footnote` element forms only, with `Label`'s key awkwardly read from children text. | **(a) Four inline sugars**, all defined as *rewrites to existing element forms* (so they inherit the element machinery — R11 line clamp, R12 positional rules, bounded-frame clipping — instead of new extent rules): `<ident>` ≡ `@Label[id: "ident"]{}`; `&ident` ≡ `@Ref[id: "ident"]{}`; `[^ident]` ≡ `@FootnoteMark[label: "ident"]{}`; line-start `[^ident]: body` ≡ `@FootnoteText[label: "ident"]: body` (the colon-body extent rule verbatim; multi-line definitions use the explicit component form). **label charset (re-amended 2026-07-05 — Typst minus period; supersedes both the original Typst-like set and the brief JS-IdentifierName amendment):** start `[A-Za-z0-9_]`, continue `[A-Za-z0-9_:-]` — the Typst label set **without `.`** (so `&sec.` never glues trailing punctuation), ASCII-only. Digits may start a label (`[^1]`, Markdown-style); kebab labels work (`<sec-intro>`, `&sec-intro`); the start restriction keeps arrow-like prose literal (`<->`, `<-x>` are text). `$` and Unicode are not label chars — the element forms are charset-free (`@Label[id: "π.α"]{}` accepts any string); only the *sugar* is restricted. **Left-boundary guard on `<` and `&` only:** the sigil fires iff preceded by start-of-body/line, whitespace, or opening punctuation (`(`/`[`/`{`/quote) — so `Vec<T>`, `R&D`, `a<b`, `a&b` stay literal prose; `[^` needs no guard (the two-char digraph is unambiguous, and `text[^1]` glues per Markdown). `<ident>` must close with `>` on its opening line (R11-consistent); `&ident` ends at the first non-ident char; a non-matching open (`< b`, `<.x>`, `&,`) is literal text. **Escapes:** `\<`, `\&`, `\[` yield literals via the standard escape machinery. Sugars fire only in markup-text position (never inside raw spans / embedded JS — the R12 condition-1 classification). Faithful AST nodes + lowering to free ambient identifiers (the R18f `HEADING`-const pattern); highlight spans classified; TextMate grammar parity. **(b) Labeled footnotes** (amends R18e): `FootnoteMark[label]` is the reference (a `"footnote"`-kind mark); `FootnoteText[label]{content}` is the definition — contributes content, renders nothing in place. Markdown semantics: repeated references to one label share ONE number (numbering = distinct-label first-appearance order, interleaved with anonymous `@Footnote{…}` one-shots, which remain supported via synthetic labels); the list renders one entry per distinct referenced label, backlinking the **first** reference (v1); a referenced label with **no** definition → pointed error; **duplicate** definitions → pointed error; an **unreferenced** definition is dropped silently (Markdown semantics — drafts accumulate). **Entry content decodes as flow (amended 2026-07-05):** the list wraps each entry's content in a `div` flow container inside its `<li>` (`li` is contractually *tight*, §7 — it would swallow paragraph breaks), with the backlink appended *inside* so it joins the final paragraph run — a blank line inside a definition body is therefore a real paragraph break (`<li><div><p>…</p><p>… ↩</p></div></li>`); the reader already emits the §7 break marker inside colon bodies. `Label`/`Ref` take an **`id` prop** (`@Label[id: "sec"]{}` / `@Ref[id: "sec"]{}`) — the children-text key form is retired. **(c) The whole doc-state family is ambient** (amends §9 + the R14 list; "the prelude should be a prelude"): `Heading, Toc, Label, Ref, Footnote, FootnoteMark, FootnoteText, Footnotes, FootnotesList, Cite, Bibliography` plus config `secset`/`bibset` (the R14d "prelude export, hence ambient" `lstset` precedent) join `Tex`/`CodeInline`/`CodeBlock`/`lstset`/`mathset` in every integrator surface (CLI inject, vite ambient-name set, language-server preamble, playground scope). `%import` shadowing still overrides per-document (R14b). **(d) Staged:** prelude (b) ∥ reader (a) — disjoint path sets — then integration (c) + goldens/docs. |
| R21 | `[props]` composes with a colon body | new (**pinned 2026-07-05**) — `@head[props]: body` mis-parsed: `parse_element`'s post-`]` continuation recognized `{` (braced body), `[` (another prop group), and `\|{` (verbatim, R19), but a glued `:` fell through — the element self-closed and `: body` leaked out as literal sibling text (the long-standing TODO.md `@p[:]` gap; it also blocked the R20b element form `@FootnoteText[label: "x"]: body`, leaving the `[^x]:` sugar as the only colon-body definition surface). | **`[props]` groups compose with a colon body exactly as with a braced or verbatim body** (the R19 move, completed for `:`): `@head[props]*: body` opens the same colon body a bare `@head: body` would — the **identical R12 positional trigger** (markup-body child + line-start-modulo-whitespace, judged at the `@`) and the **identical colon-body extent machinery**, with props threaded through unchanged (`NotaElement` already carries `props` + `children`). Where the R12 gate fails, the whole form is dead exactly as for bare heads: the head + props interpolate and `: …` stays literal text. Emit: `@aside[class: "x"]: body` → `h("aside", { class: "x" }, ["body"])`. |
| R22 | Typed emit surface (IDE) | new (**pinned 2026-07-06**) — the runtime `h` was `h(t: ElementVNode["tag"], p: Record<string, unknown> \| null, …)`: props were `unknown`-typed, so a `.nota` got no prop completion/checking, and the language-server preamble had to type the ambient prelude slots (`Tex`/`Heading`/…) with an **index-signature** `SLOT` type purely to dodge a contravariant tag-assignability failure at `h(Tex, …)` — losing the slots' real prop types. Separately, the preamble `import { … } from "@nota-lang/runtime"` bound to nothing for a `.nota` **outside** `packages/*` (no root `node_modules/@nota-lang`), so `blockComponent` et al. were `any` ("no inferred type"). | **The runtime `.d.ts` gets the typed surface, and the language-server preamble is generated from it, resolution-independently.** **(a) Two `h` overloads** (no loose `Record` overload — it would swallow wrong-value errors), tried in order: a **function tag** `h<P>(t: (props: P) => unknown, p: OmitChildren<P> \| null, …)` (a component from `inlineComponent`/`blockComponent`, or a prelude slot — its own parameter type drives `P`, so a slot's real prop type flows with no contravariance workaround; `OmitChildren` is a *key-remapped* children removal that, unlike built-in `Omit`, preserves named prop types through a permissive `[prop: string]: unknown` index) and a **string tag** `h<K extends string>(t: K, p: NotaHostProps<K> \| null, …)` where `NotaHostProps` indexes a **Nota-owned** per-tag attribute map (`NotaIntrinsicElements` — common elements seeded with their distinctive attributes over a permissive `NotaGlobalAttributes` global-attribute base + string index fallback, so `@a[href]` completes/checks `href`, `nota-*`/custom tags stay legal). A wrong prop *value* on a known tag is a real type error (surfaced as "no overload matches" on the synthesised `h(` call — generated boilerplate, so it maps to no `.nota` range; prop **completion** on the mapped prop region is the editor affordance, R22 P5). **(b) `Fragment`/`decode`/`inlineComponent`/`blockComponent` keep their real signatures.** Types only — runtime behaviour is unchanged (types erased). **(c) The preamble is generated** (`packages/language-server/scripts/gen-preamble.ts` → committed `preamble.generated.ts`, drift-guarded) by inlining the runtime's built `.d.ts` closure (`{dom,raw,doc,vnode,component,h}`) as **module-local ambient declarations** (`export` stripped, relative imports removed) rather than a `declare module`/`import` — the emit's free `h`/`decode`/… identifiers resolve to them with **no module resolution and no `node_modules`** (the fix for "`blockComponent` has no inferred type"). The prelude slots + config fns + `useState` are appended as ambient globals with their real prop shapes. The whole-lines-only preamble-shift invariant (§9 / `PREAMBLE_LENGTH`) holds. |
...
| `<sec-intro>` | `h(Label, { id: "sec-intro" }, [])` (R20 sugar; Typst-minus-`.` label; boundary-guarded — `Vec<T>`, `<->` stay text) |
| `&sec-intro` | `h(Ref, { id: "sec-intro" }, [])` (R20; boundary-guarded — `R&D` stays text; `&sec.` never glues the `.`) |
| `[^1]` | `h(FootnoteMark, { label: "1" }, [])` (digit-start labels are legal, Markdown-style) |
| `[^note1]` | `h(FootnoteMark, { label: "note1" }, [])` (R20; glues after a word like Markdown) |
| `[^note1]: See *also*…` | `h(FootnoteText, { label: "note1" }, ["See ", h("strong", {}, ["also"]), "…"])` (line-start only; colon-body extent) |
| `@aside[class: "x"]: body` | `h("aside", { class: "x" }, ["body"])` (R21: props compose with a colon body; same R12 gate as bare `@head:`) |
...
not `Math` — R14; `Heading` per R18f). **Staging (R18g):** the `Heading` rows above are now the
reader's **actual** emit — the R18g phase-3 reader re-lowering has landed (`#` sugar →
`h(Heading, …)`; a raw `@h1{…}` stays `h("h1", …)`), and the integrators supply `Heading` ambiently
alongside `Tex`/`CodeInline`/`CodeBlock`.
...
**Ambient prelude (LOCKED; amended by R14, R18, R20).** The emitted module references `useState`
and the prelude surface as **free identifiers** — ambient, per §3.1 mechanism-not-policy. The
**integrator supplies them**; the CLI does so via **esbuild `inject`** of a prelude module
(`useState` = the framework's; the rest re-exported from `@nota-lang/prelude`). The ambient set
(R20c — "the prelude should be a prelude") is the **whole** prelude surface: the slots
`Tex`/`CodeInline`/`CodeBlock`/`Heading`/`Toc`/`Label`/`Ref`/`Footnote`/`FootnoteMark`/
`FootnoteText`/`Footnotes`/`FootnotesList`/`Cite`/`Bibliography` (registry slots over
KaTeX/shiki/doc-state defaults — R14, R18e/f) and the config fns
`lstset`/`mathset`/`secset`/`bibset`. Users
