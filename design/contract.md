...
| `$a_@i$` | `h(Math, {}, [String.raw`a_@i`])` (bare `@` is literal — R13; only `\|@` arms) |
| `$a_\|@i$` | `h(Math, {}, [String.raw`a_`, i])` (armed interpolation, a sibling part) |
| `$$x^2$$` (in prose) | `h(Math, {}, [String.raw`x^2`])` (run-2 **inline**, not display — R13) |
| `$$⏎…⏎$$` (standalone fence) | `h(Math, { display: true }, [String.raw`…`])` |
