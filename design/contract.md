...
| `<sec_intro>` | `h(Label, { id: "sec_intro" }, [])` (R20 sugar; JS-ident label; boundary-guarded — `Vec<T>` stays text) |
| `&sec_intro` | `h(Ref, { id: "sec_intro" }, [])` (R20; boundary-guarded — `R&D` stays text; `&sec.` never glues the `.`) |
