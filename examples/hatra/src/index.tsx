...
export let Paper: React.FC = _ => {  
  return <Document>
    ...
    <Section title="Introduction">
      ...
      <p>The principal question underlying this work is: <em>how much effort does it take to understand a PL paper?</em> In the example above, a key challenge is understanding the notation and the concepts they represent. For a formal system as small as the simply-typed lambda calculus, such an explanation only needs to reference a few symbols. The <Smallcaps>T-Lambda</Smallcaps> rule uses 11 distinct symbols (<$>x</$>, <$>e</$>, <$>\tau</$>,  <$>\lambda</$>,  <$>.</$>,  <$>\rightarrow</$>,  <$>\Gamma</$>, <$>\vdash</$>, <$>{r`\text{\textemdash}`}</$>)  to reference 8 distinct concepts (variables, expressions, types, functions, function types, typing contexts, typing judgments, and inference rules).</p>
    ...
    <Section title="demo" name="sec:demo">
      <Commentary Document={SlicerPaper} comment_width={350}>
        ...
        <Comment selector={'#syntax-diagram'}>
          This diagram doesn't have to be drawn in Illustrator! It uses HTML for the layout and boxes, and SVG to draw the lines. That means the underlying structure is still preserved, so the syntax elements are inspectable (as opposed to a static image).
        </Comment>
