...
<Definition name="tex:tc">
  Expressions are typechecked via the judgment $\tc{\fenv}{\tyenv}{\stackenv}{\expr}{\ty}{\stackenv'}$, read as: "$\expr$ has type $\ty$ under contexts $\fenv, \tyenv, \stackenv$ producing new context $\stackenv'$."
</Definition> 
$\tyenv$ contains function-level type and provenance variables. $\stackenv$ maps variables to types and provenances to pointed-to place expressions with ownership qualifiers. For instance, when type checking `*b := a.1` in @[fig:oxide_syntax_example] , the inputs would be $\tyenv = \tyenvempty$ (empty) and $\stackenv = \{a \mapsto (\uty, \uty),~ b \mapsto \eref{\uniq}{\r_2}{\uty},~ r_1 \mapsto \{\loanform{\uniq}{a.0}\},~ \r_2 \mapsto \{\loanform{\uniq}{a.0}\}\}$.
