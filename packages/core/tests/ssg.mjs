...
try {
  ...
  const { body, state, scopedBody, plainBody } = mod.run();
  ...
  writeFileSync(join(pkgRoot, "tests/.built/body-plain.html"), plainBody);
  console.log(
    "wrote tests/.built/{body.html,state.json,body-scoped.html,body-plain.html}"
  );
} finally {
  await server.close();
}
