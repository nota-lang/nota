/**
 * The **typing preamble** prepended to every virtual `.tsx` (implementation.md §5.2, contract §9).
 *
 * The reader's `compileVirtual` emits a *bare* `.tsx` — it deliberately omits the
 * `@nota-lang/runtime` import (contract §1: "The reader does **not** emit the `@nota-lang/runtime`
 * import — the compiler shim/integrator prepends it") **and** the ambient prelude bindings the emit
 * references as free identifiers (`useState`, and the documented `CodeInline`/`CodeBlock`/`Math`
 * prelude components — contract §9 "Ambient prelude"). For the TS language service to type-check the
 ...
 *
 * **The preamble-shift rule (contract §9, the load-bearing invariant).** Prepending text to `code`
 * pushes every generated offset forward by `PREAMBLE.length` bytes. The reader's
 * `generatedOffsets` index the *bare* `.tsx`; after the prepend they must be shifted by
 * `PREAMBLE.length`. `sourceOffsets` index the `.nota` and are **unchanged** (the source is
 * untouched). {@link shiftMappings} performs exactly this.
 *
...
/**
 * The runtime import the reader omits (contract §1) — byte-identical to
 ...
 * import (contract §9 "Ambient prelude (LOCKED)"):
 *
 ...
 *   setter type-check.
 * - `CodeInline` / `CodeBlock` / `Math` — the documented prelude *components* (contract §3: `` `@x` ``
...
/** The byte length every `generatedOffsets` entry is shifted by (contract §9). */
