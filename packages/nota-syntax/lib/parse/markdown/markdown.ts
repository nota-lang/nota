...
export class MarkdownParser extends Parser {
  ...
  constructor(
    ...
    readonly skipContextMarkup: {
      readonly [type: number]: (bl: CompositeBlock, cx: BlockContext, line: Line) => boolean;
    },
    /// @internal
...
export class InlineContext {
  ...
  addDelimiter(type: DelimiterType, from: number, to: number, open: boolean, close: boolean) {
    return this.append(
      new InlineDelimiter(type, from, to, (open ? Mark.Open : 0) | (close ? Mark.Close : 0))
    );
  }
  ...
  resolveMarkers(from: number) {
    for (let i = from; i < this.parts.length; i++) {
      ...
      for (; j >= from; j--) {
        ...
        if (
          !(part instanceof InlineDelimiter && part.side & Mark.Open && part.type == close.type) ||
          (emp &&
