...
// reader emits neither (the shim/integrator supplies them — contract §1). Regenerate with:
//   cargo run -q -p oxc --example nota_compile --features codegen -- integration/golden.nota
import { Fragment, decode, h, inlineComponent } from "@nota-lang/runtime";
import { useState } from "react";

export let Colorized = inlineComponent((children) => {
	let [color, setColor] = useState("red");
	return decode(h("span", {
