...
export let Author: React.FC = ({children}) => {
  let ctx: AuthorData = {};
  let Inner = () => <div className='author'>
    {ctx.name ? <span className='author-name'>{ctx.name}</span> : <InlineError>No author name!</InlineError>}
    {ctx.affiliations ? <span className='author-affiliation'>, {
      ctx.affiliations.map((affiliation, i) => <span key={i}>
  ...
  return <>
    <AuthorContext.Provider value={ctx}>
      {children}
